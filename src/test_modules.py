...
class _zeroed_expert_bias:
    """with ブロックの間だけ、全 MoE 層の expert_bias を一時的に 0 にする。

    aux-loss-free で学習すると expert_bias が負荷分散のために働くが、これは
    top-k の「選択」に効くので、可視化すると「内容」ではなく「均等化の都合」で
    散らばって見える。評価集計の間だけ bias を 0 にすると、純粋に affinity
    （＝内容）だけで選ばれた専門家を観察できる。抜けるときに必ず元へ戻す。
    """
    def __init__(self, model):
        self.moes = [m for m in model.modules() if isinstance(m, dsm.MoE)]
        self.saved = None

    def __enter__(self):
        self.saved = [m.expert_bias.clone() for m in self.moes]
        for m in self.moes:
            m.expert_bias.zero_()
        return self

    def __exit__(self, *exc):
        for m, b in zip(self.moes, self.saved):
            m.expert_bias.copy_(b)
        return False
...
def collect_expert_usage(model, args, ds, position=None, layer=None, ignore_bias=False):
    """演算ごとに、指定位置（既定は「=」）で選ばれた専門家の回数と正解率を返す。

    ignore_bias=True にすると、集計の間だけ expert_bias を 0 にして、
    負荷分散バイアスを外した「内容（affinity）だけ」のルーティングを観察する。
    aux-loss-free で学習したモデルの専門化を、均等化の都合を除いて見たいときに使う。
    （学習済みの expert_bias は集計後にそのまま元へ戻すので、モデルは変化しない。）
    """
    if position is None:
    ...
    accuracy = {}
    bias_ctx = _zeroed_expert_bias(model) if ignore_bias else contextlib.nullcontext()
    with bias_ctx:
        for op in ds.OPS:
