...
# ---------------------- Utils ----------------------
def load_module_from_path(path, module_name="kyouzai_mla"):
...
def decode_ids(ids: torch.Tensor, itos: dict):
    return "".join(itos[int(i)] for i in ids)
    

class CharLevelDataset(Dataset):
    """
    Character-level dataset producing (input_ids[T], target_ids[T])
    target is input shifted by 1.
    """
    def __init__(self, encoded: torch.Tensor, seq_len: int):
        super().__init__()
...
def make_aozora_corpus(urls):
    ...
    # 保存ファイル一覧を取得
    files = [f for f in os.listdir(CORPUS_DIR) if f.endswith("_no_ruby.txt")]
